...
std::vector<torfile_list> tor::gettor(int argc,const char* argv)
{
	...
	char* argvv = new char[strlen(TORRENT_LOCATION)+strlen(argv)+1];
...
std::vector<torfile_list> getList(int a, const char* b)
{
	tor mytor;
...
//		path = path_tmp;
	...
	std::cout<<"File Name :"<<torrent_name + "/" +path_tmp<<std::endl;	
	for(int i =0; i< list.size(); i++)
	{
//		std::cout<<"List ["<<i<<"] = "<<list[i].name<<std::endl;

		if(list[i].name == torrent_name+"/"+path_tmp)
		{
			return i;
		}
	}
